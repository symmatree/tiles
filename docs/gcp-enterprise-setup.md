...
This creates the projects, sets up workload identity, and publishes the OIDC provider name and service account email as the `WORKLOAD_IDENTITY_PROVIDER` and `SERVICE_ACCOUNT_EMAIL` repository variables, which is what the workflows read. The same two values are also written to 1Password as `gh_oidc_workload_identity` for reading by hand.
