...
Each **Proxmox cluster node** can run an **unprivileged LXC** with **Grafana Alloy**, bind-mounting the host root read-only at `/host`, the host's real `/proc`+`/sys` at `/host/proc`+`/host/sys`, and its Alloy **config** over `/etc/alloy`. Alloy scrapes **unix/node_exporter-style** metrics (**host** memory/CPU via the `/host/proc` bind, **hwmon** via `/host/sys`), reads the **host systemd journal**, and ships everything to the **tiles** (prod) cluster over **OTLP HTTP**, with **`cluster="bond"`** and per-node **`hostname`** / **`instance`** labels.
...
**How an unprivileged CT reads the `0640 root:systemd-journal` journal:** a Proxmox **hookscript** (`tf/nodes/templates/alloy-journal-hook.sh`, wired via `hook_script_file_id`) runs on the host at CT **pre-start** and `setfacl`s `u:100000:rX` (the CT's mapped root) onto `/var/log/journal` (installing the `acl` package first if missing) -- so Alloy reads the journal without a privileged CT (privileged is impossible here: OCI images can't create privileged, bpg#2513). The ACL is **persistent** (ext4 xattr, survives reboots) and a **default ACL** is set so journald's newly-rotated journal files inherit it (journald merges, keeping both our `u:100000` and its own `group:adm` entries); the hookscript just re-applies it at every CT start as a backstop. The hookscript is best-effort and always exits 0 (a failing pre-start hook would abort the CT start), so a journal-ACL problem degrades to "no host logs this boot", never a down CT. It's delivered as an **executable** snippet (`file_mode = "0755"`, `upload_mode = "sftp"` -- the API upload can't set the exec bit; needs the provider `ssh` block in `main.tf`).
