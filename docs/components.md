...
- [`containers/argo-tag-watcher/README.md`](https://github.com/symmatree/tiles/blob/main/containers/argo-tag-watcher/README.md) -- in-cluster controller that pokes what has not caught up with a moved ref: refreshes Argo CD apps when the deploy tag moves, and restarts opted-in workloads when a floating image tag's digest moves.
