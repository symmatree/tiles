...
- **Node hostnames come from DHCP, not DNS.** A Talos node's name is delivered by the UniFi fixed-IP reservation over DHCP; reverse DNS (PTR, served by raconteur/the Synology) is irrelevant to naming. If after a power event a bare-metal node shows up as `talos-<rand>` instead of `acebase`/`lancer`, that is a DHCP-timing problem, **not** a DNS problem -- see [bare-metal-nodes.md: Recovery: power loss and phantom `talos-xxx` nodes](bare-metal-nodes.md#recovery-power-loss-and-phantom-talos-xxx-nodes).
