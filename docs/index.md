...
- **[bare-metal-nodes.md](bare-metal-nodes.md)**: Bare-metal Talos workers in Terraform (AMD and Intel metal schematics, UniFi, add/remove/reinstall, USB ISO). Rising (AMD) and AceBase (Intel) details live in the facts repo.
...
- **[remote-access.md](remote-access.md)**: How services are reached from outside the LAN and the identity-aware perimeter (BeyondCorp-style) that gates them: the oauth2-proxy front door (Google + email allowlist) shared by Argo CD and JupyterHub, the end-to-end WAN access path (external-dns -> `lhitw` -> UniFi 443 forward -> shared Cilium ingress -> gate -> ClusterIP app), the exposure switch, the hardening gotchas (refs #593-#604), and how to verify the gate.
