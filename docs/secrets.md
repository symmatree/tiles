...
* `jupyterhub-oauth-client` — Google OAuth 2.0 client credentials. **No cluster prefix**: both `tiles` and `tiles-test` share the same vault (`tiles-secrets`) and the same GCP OAuth app. Register both redirect URIs on the single app:
  ...
  Create in GCP Console → APIs & Services → Credentials → Create OAuth client ID → Web application. Store the result as a Login item with fields `OAUTH_CLIENT_ID` and `OAUTH_CLIENT_SECRET`. Will be automated alongside ArgoCD Dex Google login (see [#348](https://github.com/symmatree/tiles/issues/348)).
