...
- **Initial Admin Password**: Generated by ArgoCD during installation. On bootstrap, `charts/argocd-applications/install-application.sh` **best-effort syncs it** into the `argocd-{cluster_name}-admin` 1Password login item (e.g. `argocd-tiles-admin`, `argocd-tiles-test-admin`), so it is normally already there. That item must pre-exist and the service account must have write access to the cluster's vault. To retrieve or set it by hand (fallback):
