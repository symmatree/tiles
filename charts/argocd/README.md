...
- **Initial Admin Password**: Generated by ArgoCD during installation. Retrieve it from the cluster secret and, if you want it in 1Password, update the `argocd-{cluster_name}-admin` login item (e.g. `argocd-tiles-admin`, `argocd-tiles-test-admin`) manually:
