...
ArgoCD is initially bootstrapped via the CI workflow (`.github/workflows/nodes-plan-apply.yaml`, **bootstrap** / **fresh_cluster** profile) which runs [`bootstrap.sh`](bootstrap.sh):
...
- **Initial Admin Password**: Generated by ArgoCD during installation. On bootstrap, `install-application.sh` copies it into the existing 1Password login item `argocd-{cluster_name}-admin` (e.g. `argocd-tiles-admin`, `argocd-tiles-test-admin`) in vault `vault_name`. Fallback retrieval from the cluster secret:
