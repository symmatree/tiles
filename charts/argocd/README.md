...
during `nodes-plan-apply`, in the same apply that creates the cluster. It
...
Terraform then applies the `argocd-applications` Application, from
[`charts/argocd-applications-installer`](../argocd-applications-installer). Argo
CD syncs that Application, and everything else in the cluster follows from it.

**Note**: The ingress won't work properly until `external-dns` and `cert-manager` are running, but Argo CD can operate headless via CLI during initial setup. The release is installed with `wait = false`, because oauth2-proxy cannot become ready until the 1Password operator exists and Argo CD is what deploys it. Terraform applies the `argocd-applications` Application without waiting for Argo CD's controllers; Argo CD reconciles it once they start.
...
- **Initial Admin Password**: what you log in with before SSO works. Google login needs external-dns and cert-manager, which Argo CD itself deploys, so in the window after Terraform installs Argo CD and before the tree comes up, the local admin account is the only way in -- which is exactly when you are most likely to need it. Argo CD generates it into `argocd-initial-admin-secret`; Terraform reads that and writes the `argocd-{cluster_name}-admin` 1Password login item (e.g. `argocd-tiles-admin`) as a full login -- username, primary URL and password -- so the browser extension offers it on the login page. To retrieve or set it by hand (fallback):
