...
The root app-of-apps `Application` is installed by Terraform straight after this
release, from [`charts/app-of-apps`](../app-of-apps).

**Note**: The ingress won't work properly until `external-dns` and `cert-manager` are running, but ArgoCD can operate headless via CLI during initial setup. Nothing waits for the controllers: the release is installed with `wait = false` (oauth2-proxy cannot become ready until the 1Password operator exists, which Argo CD itself deploys), and the root Application is a custom resource that Argo CD reconciles whenever its controller starts.
...
- **Initial Admin Password**: Generated by ArgoCD during installation into `argocd-initial-admin-secret`. Terraform reads that Secret and writes the `argocd-{cluster_name}-admin` 1Password login item (e.g. `argocd-tiles-admin`), as a full login -- username, primary URL and password -- so the browser extension offers it on the Argo CD login page. Argo CD removes that Secret once the admin password is changed, at which point the Terraform data source has nothing to read and the apply fails until it is removed. To retrieve or set it by hand (fallback):
