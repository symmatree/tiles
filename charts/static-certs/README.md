...
| `unifi` | `morpheus` (UDM-SE) | `ssh` | 07:30 | `systemctl restart unifi-core` |
...
remote machine."* The `unifi` target therefore uses the generic `ssh` hook to write the
same two files that hook would have written, and to run the same reload command:
