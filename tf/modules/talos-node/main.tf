...
resource "proxmox_virtual_environment_download_file" "talos_iso" {
  for_each = { for cfg in var.talos_configs : "${cfg.version}-${cfg.variant}-${cfg.arch}-${cfg.schematic}" => cfg }
  ...
  file_name = "talos-${each.value.version}-${each.value.variant}-${each.value.arch}-${each.value.schematic}.iso"
  overwrite = false
}
